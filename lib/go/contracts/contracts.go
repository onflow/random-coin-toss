...
const (
	...
	filenameFlowToken             = "utility/FungibleToken.cdc"
...
func CoinToss(ftAddress, flowAddress, randHistoryAddress, prgAddress flow.Address) []byte {
	code := assets.MustAssetString(filenameCoinToss)

	code = placeholderFungibleToken.ReplaceAllString(code, "0x"+ftAddress.String())
	code = placeholderFlowToken.ReplaceAllString(code, "0x"+flowAddress.String())
	code = placeholderRandomBeaconHistory.ReplaceAllString(code, "0x"+randHistoryAddress.String())
	code = placeholderPseudoRandomGenerator.ReplaceAllString(code, "0x"+prgAddress.String())
