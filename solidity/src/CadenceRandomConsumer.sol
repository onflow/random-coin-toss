...
abstract contract CadenceRandomConsumer {
    ...
    function _requestRandomness() public returns (uint256) {
        // Identify the request by the current request counter
        ...
        // Store the request and increment the request counter
    ...
    function _fulfillRandomness(uint32 requestId) internal returns (uint64) {
        ...
        require(requestId < _requests.length, "Invalid request ID - value exceeds the number of existing requests");
        ...
        uint64 randomResult = _getRandomSource(request.flowHeight);
